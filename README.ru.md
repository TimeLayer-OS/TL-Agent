...
| `example-bundle/` | Готовый пример bundle с реальными `.tlsig` квитанциями |
...
`.tlsig` выпускается сетью TimeLayer. TL-Agent только читает и верифицирует его — никогда не записывает и не изменяет.
...
- **Сеть TimeLayer**: 3 независимых оператора, кворум 2 из 3, публичные ключи на GitHub
