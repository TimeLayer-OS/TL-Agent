...
> tl-agent check example-bundle action_read_files   # ALLOW | STOP(причина)
> ```
...
### 1. Скачать `tl-agent` — с пином версии и сверкой sha256

Никогда не запускайте неприкреплённый бинарник. Выберите конкретный релиз, скачайте
артефакт **и** опубликованный с ним `SHA256SUMS.txt`, и сверьте до запуска:
...
grep " tl-agent-linux-x86_64.zip$" SHA256SUMS.txt | sha256sum -c -   # должно быть: OK
unzip -o tl-agent.zip && chmod +x tl-agent
./tl-agent --help
```

Та же дисциплина, что в [`receipt-driven-examples/run.sh`](https://github.com/TimeLayer-OS/receipt-driven-examples/blob/main/run.sh): подменённый релиз обязан не пройти сверку до запуска.
...
## Что обеспечивает текущий релиз — и что нет

**Текущий релиз обеспечивает квитанционный гейт с привязкой к действию.** Действие выполняется только если:
