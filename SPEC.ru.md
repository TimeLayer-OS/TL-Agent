...
**Статус применения (v0.2.x).** Эти инварианты описывает квитанционный гейт v0.2.x: действие выполняется только если его envelope активен (не отозван, не истёк), действие объявлено в topology, `cert.tlcert` + `bundle.tlbundle` проходят внешний `timelayer-verifier` (точный вердикт `VALID FINAL`), **и квитанция заверяет именно это действие**: гейт пересчитывает канонический commitment намерения (схема `tl-intent/1`: sha256 над serde_json-каноническим объектом из schema, topology_id, action_id, receipt_id, receipt_type, valid_from, valid_until, previous_receipt_id, allowed_next_actions, scope — ключи по алфавиту; `status` сознательно исключён как изменяемое состояние жизненного цикла) и требует привязки через `--expect`. Envelope без привязки падает fail-closed (`UNBOUND_RECEIPT`), если явно не задан `TL_AGENT_ALLOW_UNBOUND=1`. Поля scope входят в commitment, но их рантайм-семантика пока НЕ применяется. `execution_log.jsonl` — диагностика, не доказательство исполнения. TL-Agent — квитанционный гейт и неподделываемый слой провенанса, а не песочница.
