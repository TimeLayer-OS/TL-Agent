> **This repository follows the Receipts + Brain method.** It is universal — it applies to any agentic or multi-agent project, not only TL-Agent (this repo). The owner supplies the repo, the brain-vault, and keys at the start of a session. See also the Russian edition: [`AGENTS.ru.md`](AGENTS.ru.md).
